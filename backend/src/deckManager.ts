...
const UNIQUE_MULTIPLIERS: Array<NonNullable<Card["multiplier"]>> = [
  "Captain",
  "Vice-captain",
];

const ROLE_ALIAS_MAP: Record<string, Role> = {
  //Not used roles can be deleted, for now only roles from REQUIRED_ROLES are valid
  top: "Top",
  jg: "Jgl",
  jgl: "Jgl",
  jungle: "Jgl",
...
export function createDeck(
  options: { userId?: number; slots?: Partial<Record<Role, Card | null>> } = {}
): Deck {
...
export function addCardToDeck(deck: Deck, card: Card): Deck {
  ...
  if (slots[role]) {
    throw new DeckError(
      "ROLE_ALREADY_OCCUPIED",
      `Role ${role} already has a card assigned.`,
      {
        role,
...
export function removeCardFromDeck(deck: Deck, roleInput: RoleInput): Deck {
  ...
  if (!slots[role]) {
    throw new DeckError(
      "ROLE_EMPTY",
      `Role ${role} does not have a card to remove.`,
      { role }
    );
  }
...
export function replaceCardInDeck(
  ...
  if (!slots[role]) {
    throw new DeckError(
      "ROLE_EMPTY",
      `Role ${role} is empty. Use addCardToDeck instead.`,
      { role }
    );
  }
...
export function upsertCardInDeck(
  deck: Deck,
  roleInput: RoleInput,
  card: Card
): Deck {
...
export function ensureDeckComplete(deck: Deck): CompleteDeck {
  ...
  const slots = REQUIRED_ROLES.reduce((acc, role) => {
    ...
    if (!card) {
      throw new DeckError(
        "ROLE_EMPTY",
        `Role ${role} is missing while validating completion.`
      );
    }
    acc[role] = card;
    return acc;
...
function assertMultiplierAvailability(
  deck: Deck,
