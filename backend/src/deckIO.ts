...
import { createDeck, normalizeRole, REQUIRED_ROLES, summarizeDeck } from "./deckManager";

export type DeckPayloadErrorCode = "INVALID_CARD" | "INVALID_DECK" | "INVALID_USER_ID";
...
export function parseCardPayload(payload: unknown, roleHint?: RoleInput): Card {
    if (!payload || typeof payload !== "object") {
        throw new DeckPayloadError("INVALID_CARD", "Card payload must be an object.");
    }
    ...
    if (typeof name !== "string" || name.trim() === "" || !role) {
        throw new DeckPayloadError("INVALID_CARD", "Card payload is missing mandatory fields.", {
            name,
            role: roleCandidate,
        });
    }
...
export function parseDeckPayload(payload: unknown): Deck {
    ...
    const raw = payload as UnknownRecord;
    const slotsInput = typeof raw.slots === "object" && raw.slots
    ...
    const userId = typeof raw.userId === "number" && Number.isInteger(raw.userId)
        ? raw.userId
        : undefined;

    const options: { userId?: number; slots?: Partial<Record<Role, Card | null>> } = { slots };
    if (userId !== undefined) {
...
export function parseUserId(value: unknown): number {
    ...
    if (!Number.isInteger(userId) || userId <= 0) {
        throw new DeckPayloadError("INVALID_USER_ID", "User identifier must be a positive integer.", { value });
    }
    return userId;
}

export function toDeckResponse(deck: Deck): { deck: Deck; summary: DeckSummary } {
    return {
        deck,
